...
5. [Работа над ошибками](https://github.com/Neytrin/Network-ingeneer/blob/main/labs/lab06/readme.md#5-%D1%80%D0%B0%D0%B1%D0%BE%D1%82%D0%B0-%D0%BD%D0%B0%D0%B4-%D0%BE%D1%88%D0%B8%D0%B1%D0%BA%D0%B0%D0%BC%D0%B8)
...
![R14 sh_OSPF_int.png](R14%20sh_OSPF_int.png)

sh ip protocols 

![r14 sh_ip_protocols.png](r14%20sh_ip_protocols.png)

Установлено соседство с роутерами R12, R13

![R14 ip ospf neighbor.png](R14%20ip%20ospf%20neighbor.png)
...
![R14 sh ip route.png](R14%20sh%20ip%20route.png)
...
![R14 sh ip route 2.png](R14%20sh%20ip%20route%202.png)
...
````
![R14 total_stub.png](R14%20total_stub.png)

В подтверждение выполнения условий отобразим таблицу маршрутизации для R19

![R19 ip route.png](R19%20ip%20route.png)
...
![R20 ping R19.png](R20%20ping%20R19.png)
...
![R20 ip route.png](R20%20ip%20route.png)

Вывод содержимого LSDB R20
![R20 LSDB.png](R20%20LSDB.png)
...
![R20 ip route 2.png](R20%20ip%20route%202.png)
![R20 LSDB2.png](R20%20LSDB2.png)
...
Таким образом командой _**default-information originate**_ передали не только default-route R14 и R15, но и назначили маршрутизаторам роль ASBR
...
![IPv6 R20 LSDB.png](IPv6%20R20%20LSDB.png)

Вывод таблицы маршрутизации IPv6 на R20
![IPv6 Route OSPF R20.png](IPv6%20Route%20OSPF%20R20.png)
...
#### Вопрос 1. Повышение отказоустойчивости за счет избыточности соединений в зоне 0.

Для иллюстрации приведена измененная схема.

![SHEM_OSPF_correct.png](Correct%2FSHEM_OSPF_correct.png)

Внесены дополнения в таблицу адресов для новых соединений R14-R15 и R12-R13.
...
В конфигурации маршрутизаторов R12-R15 внесены соответствующие изменения, дополнительно настроены интерфейсы e1/0 и включены в процесс 
OSPF в пределах Area 0.

Проведем сравнительный анализ полученных результатов с ранее полученными на предыдущими настройках. 
Для наглядности снова выбран R14. 
Покажем, что:
Интерфейс e1/0 участвует в OSPF
![R14 OSPF_int_correct.png](Correct%2FR14%20OSPF_int_correct.png)

Добавилось соседство с R15.
![R14 OSPF_neib_correct.png](Correct%2FR14%20OSPF_neib_correct.png)

Получены новые и дополнительные маршруты по OSPF.
![R14_IP_route_correct.png](Correct%2FR14_IP_route_correct.png)

#### Вопрос 2. Фильтрацию маршрутов IPv6 из Area 101 в сторону Area 102.

Требовалось выполнить настройки по аналогии с IPv4.
На R15 ранее IPv6 router OSPF имел настройки
````
router ospfv3 1
  address-family ipv6 unicast
 exit-address-family
````
при этом не было возможности настроить фильтрацию LSA3 (нет функции filter-list)
В новой настройке IPv6 router OSPF
````
ipv6 router ospf 1
...
````
Фильтрация осуществляется на ABR R15 на входе в Area 102 c использованием prefix-list.
Демонстрация отсутствия маршрута в сеть 203A:BB8A:D701:4::/64 в таблице маршрутизации R20.

![R20 IPv6_route_correct.png](Correct%2FR20%20IPv6_route_correct.png)
