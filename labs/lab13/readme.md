...
1. [Настроить GRE между офисами Москва и С.-Петербург.](https://github.com/Neytrin/Network-ingeneer/blob/main/labs/lab13/readme.md#1-%D0%BD%D0%B0%D1%81%D1%82%D1%80%D0%BE%D0%B8%D1%82%D1%8C-gre-%D0%BC%D0%B5%D0%B6%D0%B4%D1%83-%D0%BE%D1%84%D0%B8%D1%81%D0%B0%D0%BC%D0%B8-%D0%BC%D0%BE%D1%81%D0%BA%D0%B2%D0%B0-%D0%B8-%D1%81-%D0%BF%D0%B5%D1%82%D0%B5%D1%80%D0%B1%D1%83%D1%80%D0%B3)
2. [Настроить DMVPN между Москва и Чокурдак, Лабытнанги.](https://github.com/Neytrin/Network-ingeneer/blob/main/labs/lab13/readme.md#2-%D0%BD%D0%B0%D1%81%D1%82%D1%80%D0%BE%D0%B8%D1%82%D1%8C-dmvpn-%D0%BC%D0%B5%D0%B6%D0%B4%D1%83-%D0%BC%D0%BE%D1%81%D0%BA%D0%B2%D0%B0-%D0%B8-%D1%87%D0%BE%D0%BA%D1%83%D1%80%D0%B4%D0%B0%D0%BA-%D0%BB%D0%B0%D0%B1%D1%8B%D1%82%D0%BD%D0%B0%D0%BD%D0%B3%D0%B8)
...
````
В качестве источника и назначения выбраны интерфейсы e0/2 роутеров включенные в сторону провайдеров.
как показано на рисунке ниже

![graf_gre.png](graf_gre.png)

Проверяем таблицы маршрутизации на примере R15 и доступность IP адреса назначенного при настройке туннеля на стороне R18.

![R15 sh_ip_route tunnel.png](R15%20sh_ip_route%20tunnel.png)

![R15 ping R18_tunnel.png](R15%20ping%20R18_tunnel.png)

Допустим мы хотим связать через GRE туннель две сети офисов в которых находятся VPC7 (Москва) и VPC (С.-Петербург).
На роутерах R15 и R18 создаем статические маршруты

````
R15(config)#ip route 172.18.2.0 255.255.255.0 Tunnel1
````
и 
````
R18(config)#ip route 172.16.12.0 255.255.255.0 Tunnel1
````

Осуществляем проверку доступности хостов между офисами по туннелю

![VPC7 ping VPC.png](VPC7%20ping%20VPC.png)

![VPC ping VPC7.png](VPC%20ping%20VPC7.png)

На этом настройку можно было считать завершенной, но появляется вопрос надежности соединения между сетями офисов.
Ведь каждый из офисов имеет по два соединения с провайдерамиа в нашем случае возникновения проблем на стороне провайдера или пропадания 
линка между офисом и провайдером соединение сетей по организованному туннелю прервется.
Например так

![graf_gre_fail.png](graf_gre_fail.png)

![R15 ping R18 fail tunnel.png](R15%20ping%20R18%20fail%20tunnel.png)

Повысить надежность можно настройкой дополнительных туннелей, как показано ниже

![graf_gre add.png](graf_gre%20add.png)

Есть более изящный способ решения. Можно в качестве источников и назначения при настройке туннеля использовать уже настроенные интерфейсы 
Loopback1 на R15 и R18. Им назначены публичный IP адреса из пулов собственных AS.

````
R15(config-if)#tunnel source Loopback1
R15(config-if)#tunnel destination 113.201.100.1

````
````
R18(config-if)#tunnel source loopback 1
R18(config-if)#tunnel destination 98.10.10.1
````
Использование данного подхода при настройке туннеля не требует дополнительных избыточных записей в конфигурации оборудовании.

При имитации повреждения на одном из соедениний с провайдером Ламас, соединение между сетями двух офисов прерывалась на время перестроения 
маршрутов полученных по протоколу eBGP. 

Выполняем проверку.
Со стороны R15 туннель выглядит вполне рабочим

![R15 ping tunnel to R18.png](R15%20ping%20tunnel%20to%20R18.png)

Однако нет соединения между сетями 172.16.12.0/24(Москва) и 172.18.2.0/24(C.Петербург)

![VPC7 ping VPC fail.png](VPC7%20ping%20VPC%20fail.png)

![VPC ping VPC7 fail.png](VPC%20ping%20VPC7%20fail.png)

Все потому, что R14 не знает о существовании туннеля и нет маршрута для сети 172.18.2.0/24

Включим интерфейс Tunnel1 в процесс OSPF
````
R15(config-if)#ip ospf 1 area 0
````
На R14 cоздадим статический маршрут в сторону интерфейса Tunnel1 на R15
````
R14(config)#ip route 172.18.2.0 255.255.255.0 10.0.1.1
````
Проверяем

![VPC7 ping VPC cool.png](VPC7%20ping%20VPC%20cool.png)

![VPC ping VPC7 fail.png](VPC%20ping%20VPC7%20fail.png)

Заключение.

С точки зрения повышения надежности работы туннеля при повреждении одного из соединений на провайдера 
достигнута определенная отказоустойчивость. Примененное решение абсолютно подходит к варианту подключения к провайдерам реализованном 
для офиса С.-Петербург и не совсем оптимально для Москвы. В случае выхода из строя R15, тунель работать не будет, не смотря на то,
что сеть офиса останется вполне работоспособной. Возможно в случае с офисом Москва было-бы оптимальным настроить дополнительный туннель
между R14 и R18. 

### 2. Настроить DMVPN между Москва и Чокурдак, Лабытнанги.
...
Все изменения в настройках оборудования приведены [здесь](https://github.com/Neytrin/Network-ingeneer/tree/main/labs/lab13/Configs)