...
R1#
````
Ну и на последок, все ради чего настраивался протокол OSPF, состояние таблицы маршрутизации с выводом только маршрутов полученных по OSPF для R1.
````
R1# sh ip route ospf
...
#### 12. Построение динамического туннеля.
...
````
Получили маршрут R5(e0/2)-R7(e0/0)-R9(e0/1). Добились чего желали, но есть нюанс, что для данной топологии и конкретно 
...
#### 16. Построить L3VPN в тунеле MPLS-TE.
